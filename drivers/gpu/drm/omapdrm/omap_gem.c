...
static int _omap_gem_pin(struct drm_gem_object *obj)
{
	...
	if (!priv->has_dmm || !(omap_obj->flags & OMAP_BO_MEM_SHMEM))
		return -EINVAL;
	...
	ret = tiler_pin(block, omap_obj->pages, obj->size >> PAGE_SHIFT,
			omap_obj->roll, true);
	if (ret) {
		tiler_release(block);
		dev_err(obj->dev->dev, "could not pin: %d (%d)\n", ret, fmt);
		return ret;
	}
...
static void _omap_gem_unpin(struct drm_gem_object *obj)
{
	...
	if (omap_gem_is_contiguous(omap_obj))
		return;  /* implicitly pinned */
	if (WARN_ON(!omap_obj->dma_addr_cnt))
		return;  /* not pinned! */
	if (--omap_obj->dma_addr_cnt)
